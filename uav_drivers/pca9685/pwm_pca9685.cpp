...
public:
    ...
    const hardware_interface::HardwareComponentInterfaceParams & params) override
  {
    ...
    // reserve buffer to store register values for all ON/OFF counts
    count_on_off_buffer.resize(nrotors * 4);
    ...
    if (i2c_smbus_write_byte_data(fd, R_MODE1, 0b1000'0000) < 0)
    {
      RCLCPP_FATAL_STREAM(get_logger(), "Failed to set RESTART bit: " << strerror(errno));
      return hardware_interface::CallbackReturn::ERROR;
    }
  ...
  {
    // reset
    static constexpr std::array<uint8_t, max_pwm_channls * 4> stop_buffer{};
    if (i2c_smbus_write_block_data(fd, R_LED0_ON_L, stop_buffer.size(), stop_buffer.data()) < 0)
    {
    ...
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) override
  {
    for (size_t i = 0; i < nrotors; ++i)
    {
      ...
      RCLCPP_INFO_STREAM(
        get_logger(), i << " " << rotor_cmds[i] << " >> " << step_off << " / " << steps);

      count_on_off_buffer[4 * i + 0] = 0;                       // ON Low Byte (LEDx_ON_L)
      count_on_off_buffer[4 * i + 1] = 0;                       // ON High Byte (LEDx_ON_H)
      count_on_off_buffer[4 * i + 2] = (step_off >> 0) & 0xFF;  // OFF Low Byte (LEDx_OFF_L)
      count_on_off_buffer[4 * i + 3] = (step_off >> 8) & 0x1F;  // OFF High Byte (LEDx_OFF_H)
    }

    if (
      i2c_smbus_write_block_data(
        fd, R_LED0_ON_L, count_on_off_buffer.size(), count_on_off_buffer.data()) < 0)
    {
...
private:
  ...
  std::vector<uint8_t> count_on_off_buffer;
