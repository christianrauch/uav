...
public:
    ...
    const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/) override
  {
    if (const std::optional<mavros_msgs::msg::OverrideRCIn> msg = msg_rc.try_get())
    {
      ...
      {
        if (
          msg->channels[i] == mavros_msgs::msg::OverrideRCIn::CHAN_RELEASE ||
          msg->channels[i] == mavros_msgs::msg::OverrideRCIn::CHAN_NOCHANGE)
        {
          continue;
        }

        // scale from [1000, 2000] to [0, 1]
        set_state<double>(
          std::string{sensor_name} + "/" + std::string{state_prefix} + "/" + std::to_string(i + 1),
          (msg->channels[i] - 1000) / 1000.0);
      }
    }
